...
func main() {
	...
	action := flag.String("action", "list", "list,import,generate,generateAndImport,generateSecret,generateAES,generateDES,unwrapECWithDES3,unwrapECWithAES,wrapKeyWithDES3,getSKI,setSKIFromFile, SignHMAC384,TestAESGCM,generateCSR,importCert,deleteObj")
	...
	case "getSKI":
		if *keyType == "RSA" {
			...
			exitWhenError(err)
			key.GenSKI()
			fmt.Printf("SKI(sha256): %s : %x\n", key.SKI.Sha256, key.SKI.Sha256Bytes)
			os.Exit(0)
		}
	...
	case "unwrapECWithDES3":	
		...
		if *keyType == "EC" {
			err := p11w.UnWrapECKeyFromFile(*keyFile, *keyStore, *keyStorepass, *keyLabel, w[0], "DES3")
			exitWhenError(err)
		}

	case "unwrapECWithAES":	
		w, _, err := p11w.FindObjects([]*pkcs11.Attribute{
			pkcs11.NewAttribute(pkcs11.CKA_LABEL, *wrapKey),
			},
			1,
		...
		if *keyType == "EC" {
			err := p11w.UnWrapECKeyFromFile(*keyFile, *keyStore, *keyStorepass, *keyLabel, w[0], "AES")
			exitWhenError(err)
		}
